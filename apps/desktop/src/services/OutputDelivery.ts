...
export class TauriOutputDelivery implements OutputDelivery {
  ...
  async saveToFile(
    ...
    try {
      path = await save({
        defaultPath: suggestedName.endsWith(".wiki")
          ? suggestedName
          : `${suggestedName}.wiki`,
        filters: [{ name: "Wikitext", extensions: ["wiki"] }],
      });
    } catch (cause) {
      throw new PerseusError("InputError", "Could not open the save dialog.", {
        cause,
      });
    }
    ...
    try {
      ...
      throw new PerseusError(
        "InputError",
        `Could not save the file to "${path}".`,
        { cause },
      );
    }
  ...
  async saveSession(
    ...
    try {
      path = await save({
        ...
        filters: [
          { name: SESSION_FILTER_NAME, extensions: [SESSION_EXTENSION] },
        ],
      });
    } catch (cause) {
      throw new PerseusError("InputError", "Could not open the save dialog.", {
    ...
    try {
      ...
      throw new PerseusError(
        "InputError",
        `Could not save the session to "${path}".`,
        { cause },
      );
    }
  ...
  async openSession(): Promise<TranslationSession | null> {
    ...
    try {
      ...
      throw new PerseusError("InputError", "Could not open the file dialog.", {
        cause,
      });
    }
    ...
    try {
      ...
      throw new PerseusError("InputError", `Could not read "${path}".`, {
        cause,
      });
    }
    ...
    try {
      ...
      throw new PerseusError("InputError", `"${path}" is not valid JSON.`, {
        cause,
      });
    }
