import type { ExtractionResult } from "../pipeline/Pipeline";
...
function numericSuffix(nodeId: string): number {
  ...
  if (!match) {
    throw new PerseusError(
      "GenerationError",
      `Unexpected TextNode id shape: "${nodeId}".`,
    );
  }
...
export function exportTranslationSession(
  ...
  const currentTextByNodeId = new Map(
    ir.textNodes.map((node) => [node.id, node.text]),
  );

  const sessionChunks: SessionChunk[] = chunks.map((chunk) => {
    const translation: TranslationEntryTuple[] = chunk.units.map((unit) => {
      const tag =
        ir.structure.nodeElements.get(unit.nodeId)?.tagName.toLowerCase() ??
        "unknown";
      const currentText =
        currentTextByNodeId.get(unit.nodeId) ?? unit.sourceText;
