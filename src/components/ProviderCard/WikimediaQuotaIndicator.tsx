...
export function WikimediaQuotaIndicator({ apiKey }: { apiKey: string | undefined }) {
  ...
  return (
    <div className="col-span-2 flex flex-col gap-2 rounded-md border border-border bg-card p-3">
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-foreground">{t("providerCard.quota.title")}</span>
        <Button
          ...
          variant="ghost"
        >
          {loading ? t("providerCard.quota.refreshing") : t("providerCard.quota.refresh")}
        </Button>
      </div>

      {hasError && <p className="text-xs text-destructive">{t("providerCard.quota.error")}</p>}
      ...
      {!hasError && quota && (
        <>
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            ...
            <span>
              {t("providerCard.quota.remaining", { remaining: formatCost(quota.remainingCost) })}
            </span>
          </div>
      ...
      {!hasError && !quota && loading && (
        <p className="text-xs text-muted-foreground">{t("providerCard.quota.loading")}</p>
      )}
    </div>
  );
}
