...
import { TauriOutputDelivery } from "@/services/OutputDelivery";
...
export function useTranslationSession({
  ...
  const openSession = useCallback(async (): Promise<Map<
    string,
    string
  > | null> => {
    resetSession();
    ...
    try {
      ...
      const pristineTextByNodeId = new Map(
        extraction.ir.textNodes.map((n) => [n.id, n.text]),
      );
